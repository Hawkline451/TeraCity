...
public class CodeCityProjectLoader implements CodeCityLoader {
	...
	public CodeRepresentation filesToRepresentation(final File file) {
		if (!file.isDirectory()) {
			...
			if (isJava(name)) {
				return new CodeClass(name, 0 /* TODO Ver variables */,
						countLines(file.getPath()) /* TODO ver largo archivo */, file.getPath(), "",
						countLineLength(file.getPath()), transformToCodeRepresentation(file.getPath()));
			} else
